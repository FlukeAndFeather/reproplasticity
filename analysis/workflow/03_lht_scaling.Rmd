...
lht_plots <- map2(
  quos(female_maturity_d, gestation_d, weaning_d, longevity_y, 
       litter_or_clutch_size_n, inter_birth_interval_y),
  c("Female age at maturity (days)", "Gestation (days)", "Weaning (days)", 
...
```{r prep_lht_phy}
# Impute LHTs as median of genus
impute_lht <- function(col, col_name, all_data) {
  if_else(is.na(col), all_data[[paste0(col_name, "_genus")]], col)
}
mammal_lht_impute <- mammal_lht_corr %>% 
  # Aggregate species' traits (taxonomic resolution may lead to lumping)
  group_by(genus, species, tree_name, adult_body_mass_kg) %>% 
  summarize(
    across(
  ...
  # Calculate genus-level traits (median)
  group_by(genus) %>% 
  mutate(
    across(
      c(female_maturity_d, gestation_d, weaning_d, longevity_y, 
  ...
  mutate(
    across(
      c(female_maturity_d, gestation_d, weaning_d, longevity_y, 
        litter_or_clutch_size_n, inter_birth_interval_y),
      ~ impute_lht(.x, cur_column(), cur_data_all()),
      .names = "{.col}_impute"
    )
  )

# Convert to matrix and drop incomplete rows
mammal_lht_mtx <- mammal_lht_impute %>% 
  select(adult_body_mass_kg, ends_with("impute")) %>% 
  as.matrix()
row.names(mammal_lht_mtx) <- mammal_lht_impute$tree_name
complete_rows <- apply(mammal_lht_mtx, 1, function(row) !any(is.na(row)))
mammal_lht_mtx <- mammal_lht_mtx[complete_rows, ]
```

See [Jeschke and Kokko (2009)](https://doi.org/10.1007/s10682-008-9276-y) *Evolutionary Ecology*

```{r size_resid}
# Calculate size residuals
lht_size_resid <- phyl.resid(mammal_tr, )
```

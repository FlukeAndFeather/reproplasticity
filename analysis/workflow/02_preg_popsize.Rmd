...
# Starting parameters
fin_data <- filter(preg_wide, 
                   CommonName == "Fin Whale")
fin_lm <- lm(log(preg_rate) ~ Pop_size, 
             data = fin_data, 
             weights = fin_data$n_f_catch)
a0 <- exp(coef(fin_lm)[1])
b0 <- -coef(fin_lm)[2]

# Fit model
# This doesn't work. Error: "non-finite finite-difference value [2]"
# ricker_mle <- optim(
#   par = c(a = a0, b = b0),
#   fn = modrickernll,
#   s = fin_data$Pop_size,
...
modrickernll2 <- function(a, b, s, ng, nf) {
  result <- modrickernll(c(a, b), s, ng, nf)
  if (result == 1e6) NA else result
}
fin_loglik_space <- expand_grid(
  a = seq(0.01, 0.99, length.out = 50),
  b = seq(1e-8, 1e-5, length.out = 50)
) %>% 
  mutate(nll = map2_dbl(a, b, 
                        modrickernll2, 
                        s = fin_data$Pop_size, 
                        ng = fin_data$n_preg, 
                        nf = fin_data$n_f_catch))
ggplot(fin_loglik_space, aes(a, b, fill = log(nll))) +
  geom_raster()
...
ggplot(fin_data, aes(Pop_size, preg_rate)) +
  geom_point(aes(size = n_f_catch), shape = 21) +
  geom_function(fun = ~ a * exp(-b * .x)) +
  theme_classic()
```
